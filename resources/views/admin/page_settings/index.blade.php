...
<style>
    ...
    .vb-icon-btn.active { background: var(--vb-primary); color: #fff; box-shadow: 0 2px 8px rgba(22,126,167,.3); }
    ...
    .vb-field input:focus, .vb-field textarea:focus {
        outline: none; border-color: var(--vb-primary); box-shadow: 0 0 0 3px rgba(22,126,167,.12);
    }
    ...
    .vb-loader .spin { width: 32px; height: 32px; border: 2.5px solid #e0f2fe; border-top-color: var(--vb-primary); border-radius: 50%; animation: vbSpin .8s linear infinite; }
    ...
    .vb-publish:hover { background: #116080; box-shadow: 0 4px 12px rgba(22,126,167,.3); transform: translateY(-1px); }
    ...
    .btn-select-icon.selected {
        border-color: var(--vb-primary);
        background: var(--vb-primary);
        color: #fff;
    }
    .vb-services-list-item {
        display: flex;
        align-items: center;
    ...
    .vb-services-list-item .item-info {
        display: flex;
        align-items: center;
        gap: 8px;
        min-width: 0;
    ...
    .vb-services-list-item .item-icon {
        color: var(--vb-primary);
        display: flex;
        align-items: center;
        flex-shrink: 0;
    }
    .vb-services-list-item .item-title {
        font-size: 11px;
        font-weight: 700;
        color: var(--vb-text);
        white-space: nowrap;
    ...
    .vb-services-list-item .item-actions {
        display: flex;
        gap: 3px;
        flex-shrink: 0;
    }
    .vb-services-list-item .action-btn {
        width: 24px;
        height: 24px;
        border-radius: 4px;
        display: flex;
    ...
    .vb-services-list-item .action-btn:hover {
        background: #f1f5f9;
        color: var(--vb-text);
    }
    .vb-services-list-item .action-btn.delete:hover {
        background: #fee2e2;
        color: #ef4444;
        border-color: #fca5a5;
    }
...
<div class="vb-root">
    ...
    <div class="vb-main">
        ...
        <nav class="vb-icon-nav">
            ...
            <button type="button" onclick="VB.switchTab('services')" id="vb-tab-services" class="vb-icon-btn">
                <span class="material-symbols-outlined">work</span><span>Layanan</span>
            </button>
        ...
        <aside id="vb-form-panel" class="vb-form-panel">
            ...
            <div class="vb-form-body vb-scroll">
                <form id="vb-form" action="{{ route('admin.page_settings.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div id="vb-panel-services" class="vb-panel">
                        <div class="vb-card">
                            <div class="vb-card-title">
                                <div class="ico"><span class="material-symbols-outlined">work</span></div>
                                <div><h4>Bidang Usaha</h4><p>Atur lini layanan / bidang usaha perusahaan</p></div>
                            </div>
                            
                            {{-- Hidden input to store JSON string --}}
                            <input type="hidden" id="in_dashboard_services" name="dashboard_services" value="{{ $settings['dashboard_services'] ?? '' }}">
                            
                            {{-- List container --}}
                            <div id="vb-services-list-container" class="vb-services-list" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 12px;">
                                <!-- Will be loaded via JS -->
                            </div>
                            
                            <button type="button" class="vb-publish" style="width: 100%; justify-content: center; background: #167ea7;" onclick="VB.openAddServiceForm()">
                                <span class="material-symbols-outlined">add</span> Tambah Layanan Baru
                            </button>
                        </div>
                        
                        {{-- Form editor card (Hidden by default) --}}
                        <div id="vb-service-form-card" class="vb-card" style="display: none; border: 2px solid var(--vb-primary);">
                            <div class="vb-card-title">
                                <div class="ico"><span class="material-symbols-outlined">edit_note</span></div>
                                <div><h4 id="vb-service-form-title">Tambah Layanan</h4><p>Lengkapi informasi layanan</p></div>
                            </div>
                            <input type="hidden" id="service-edit-index" value="">
                            
                            <div class="vb-field">
                                <label>Nama / Judul Layanan</label>
                                <input type="text" id="service-title" placeholder="Contoh: Konstruksi Mekanikal">
                            </div>
                            
                            <div class="vb-field">
                                <label>Deskripsi Singkat</label>
                                <textarea id="service-desc" rows="3" placeholder="Contoh: Fabrikasi & erection struktur mekanikal berat..."></textarea>
                            </div>
                            
                            <div class="vb-field">
                                <label>Pilih Icon</label>
                                <div class="vb-icon-selector-grid" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-top: 6px;">
                                    <button type="button" class="btn-select-icon" data-icon="cog" onclick="VB.selectServiceIcon('cog')">@include('web.partials.icon', ['name' => 'cog', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="zap" onclick="VB.selectServiceIcon('zap')">@include('web.partials.icon', ['name' => 'zap', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="wrench" onclick="VB.selectServiceIcon('wrench')">@include('web.partials.icon', ['name' => 'wrench', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="factory" onclick="VB.selectServiceIcon('factory')">@include('web.partials.icon', ['name' => 'factory', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="warehouse" onclick="VB.selectServiceIcon('warehouse')">@include('web.partials.icon', ['name' => 'warehouse', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="hammer" onclick="VB.selectServiceIcon('hammer')">@include('web.partials.icon', ['name' => 'hammer', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="shield-check" onclick="VB.selectServiceIcon('shield-check')">@include('web.partials.icon', ['name' => 'shield-check', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="badge-check" onclick="VB.selectServiceIcon('badge-check')">@include('web.partials.icon', ['name' => 'badge-check', 'size' => 18])</button>
                                    <button type="button" class="btn-select-icon" data-icon="building-2" onclick="VB.selectServiceIcon('building-2')">@include('web.partials.icon', ['name' => 'building-2', 'size' => 18])</button>
                                </div>
                                <input type="hidden" id="service-icon" value="cog">
                            </div>
                            
                            <div style="display: flex; gap: 8px; margin-top: 12px; border-top: 1px solid var(--vb-border); padding-top: 12px;">
                                <button type="button" class="vb-publish" style="background: #167ea7;" onclick="VB.saveService()">Simpan</button>
                                <button type="button" class="vb-publish" style="background: #64748b;" onclick="VB.cancelServiceForm()">Batal</button>
                            </div>
                        </div>
                    </div>
...
<div id="vb-submit-loader" style="display: none; position: fixed; inset: 0; background: rgba(15, 23, 42, 0.65); z-index: 99999; flex-direction: column; align-items: center; justify-content: center; gap: 14px; backdrop-filter: blur(4px);">
    <div style="width: 44px; height: 44px; border: 3px solid rgba(255,255,255,0.3); border-top-color: #167ea7; border-radius: 50%; animation: vbSpin .8s linear infinite;"></div>
...
const VB = {
    pages: {
        ...
        services:{ url: '{{ url('/') }}',                  title: 'Bidang Usaha',       desc: 'Kelola bidang usaha di beranda',  label: 'Bidang Usaha' },
            ...
            try {
                ...
                if (doc) {
                    if (tab === 'contact') {
                        ...
                        const target = doc.getElementById('layanan');
                        if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    } else {
    ...
    onIframeLoad(iframe) {
        ...
        try {
            ...
            // Re-apply services dynamic rendering in iframe
            if (this.currentUrl === '{{ url('/') }}') {
                this.applyServicesToIframe();
            }
            ...
            style.innerHTML = `
                ...
                .vb-edit:hover { outline: 2px dashed #167ea7; outline-offset: 4px; background: rgba(22,126,167,.05); cursor: text; border-radius: 4px; }
                .vb-edit:focus { outline: 2px solid #167ea7; outline-offset: 4px; background: rgba(255,255,255,.95); border-radius: 4px; color: #000; }
                .vb-img-edit { transition: outline .15s, filter .15s; }
                .vb-img-edit:hover { outline: 3px dashed #167ea7; outline-offset: 4px; filter: brightness(.88); cursor: pointer; }
    ...
    initServices() {
        const inputVal = document.getElementById('in_dashboard_services').value;
        try {
            this.services = JSON.parse(inputVal) || [];
        } catch(e) {
            this.services = [
            ...
            document.getElementById('in_dashboard_services').value = JSON.stringify(this.services);
        }
        this.renderServicesList();
    },

    renderServicesList() {
        const container = document.getElementById('vb-services-list-container');
        if (!container) return;
        container.innerHTML = '';

        if (this.services.length === 0) {
            container.innerHTML = '<div style="font-size: 11px; text-align: center; color: var(--vb-muted); padding: 12px; border: 1px dashed var(--vb-border); border-radius: 8px;">Belum ada bidang usaha. Silakan tambah baru.</div>';
            return;
        }

        this.services.forEach((s, idx) => {
            const item = document.createElement('div');
            item.className = 'vb-services-list-item';
            
            const iconSvg = ICON_SVGS[s.icon] || ICON_SVGS['cog'];
            
            item.innerHTML = `
                <div class="item-info">
                    <div class="item-icon">${iconSvg.replace('width="24" height="24"', 'width="16" height="16"')}</div>
                    <div class="item-title">${s.title}</div>
                </div>
                <div class="item-actions">
                    <button type="button" class="action-btn" title="Naik" onclick="VB.moveService(${idx}, -1)" ${idx === 0 ? 'disabled style="opacity:.4;"' : ''}><span class="material-symbols-outlined" style="font-size:14px;">arrow_upward</span></button>
                    <button type="button" class="action-btn" title="Turun" onclick="VB.moveService(${idx}, 1)" ${idx === this.services.length - 1 ? 'disabled style="opacity:.4;"' : ''}><span class="material-symbols-outlined" style="font-size:14px;">arrow_downward</span></button>
                    <button type="button" class="action-btn" title="Edit" onclick="VB.editService(${idx})"><span class="material-symbols-outlined" style="font-size:14px;">edit</span></button>
                    <button type="button" class="action-btn delete" title="Hapus" onclick="VB.deleteService(${idx})"><span class="material-symbols-outlined" style="font-size:14px;">delete</span></button>
                </div>
            `;
    ...
    openAddServiceForm() {
        document.getElementById('vb-service-form-title').innerText = 'Tambah Layanan Baru';
        document.getElementById('service-edit-index').value = '';
        document.getElementById('service-title').value = '';
        document.getElementById('service-desc').value = '';
        this.selectServiceIcon('cog');
        document.getElementById('vb-service-form-card').style.display = 'block';
        document.getElementById('vb-service-form-card').scrollIntoView({ behavior: 'smooth' });
    },

    selectServiceIcon(icon) {
    ...
    editService(idx) {
        ...
        document.getElementById('vb-service-form-title').innerText = 'Edit Layanan';
        ...
        this.selectServiceIcon(s.icon || 'cog');
        document.getElementById('vb-service-form-card').style.display = 'block';
        document.getElementById('vb-service-form-card').scrollIntoView({ behavior: 'smooth' });
    },
    ...
    saveService() {
        ...
        const idxVal = document.getElementById('service-edit-index').value;
        ...
        const serviceData = { title, desc, icon };

        if (idxVal !== '') {
            this.services[parseInt(idxVal)] = serviceData;
        } else {
    ...
    cancelServiceForm() {
        document.getElementById('vb-service-form-card').style.display = 'none';
        document.getElementById('service-edit-index').value = '';
        document.getElementById('service-title').value = '';
        document.getElementById('service-desc').value = '';
    },
    ...
    moveService(idx, direction) {
        const targetIdx = idx + direction;
        if (targetIdx < 0 || targetIdx >= this.services.length) return;
        const temp = this.services[idx];
        this.services[idx] = this.services[targetIdx];
        this.services[targetIdx] = temp;
        document.getElementById('in_dashboard_services').value = JSON.stringify(this.services);
        this.renderServicesList();
    },

    applyServicesToIframe() {
        try {
            const doc = document.getElementById('vb-iframe').contentDocument;
            if (!doc) return;
            const grid = doc.querySelector('.cke-services__grid');
            if (!grid) return;
            ...
            this.services.forEach((s, idx) => {
                ...
                link.href = '{{ url('layanan') }}';
                link.className = 'block text-inherit no-underline';
                
                link.innerHTML = `
                    <div class="cke-service">
                        <span class="cke-service__index">${indexStr}</span>
                        <div class="cke-service__icon">
                            ${iconSvg}
                        </div>
                        <h3 class="cke-service__title">${s.title}</h3>
                        <p class="cke-service__desc">${s.desc || ''}</p>
                    </div>
                `;
                grid.appendChild(link);
            });
        } catch(e) { console.warn('Iframe update services error:', e); }
    }
};
