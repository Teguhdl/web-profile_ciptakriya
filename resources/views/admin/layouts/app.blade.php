...
<head>
    ...
    <!-- CKE Brand Fonts (sync with public website) -->
    ...
    <!-- CKE Admin Theme (brand re-skin, must load AFTER final.css) -->
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="absolute inset-y-0 left-0 z-50 w-72 bg-white border-r border-gray-100 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 flex flex-col">
            ...
            <div class="h-20 flex items-center px-6 border-b border-gray-50">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 group">
                    <div class="h-11 w-11 flex items-center justify-center rounded-xl overflow-hidden bg-white border border-gray-100 shadow-sm">
                        @php
                            $logo = \App\Models\Setting::where('key', 'system_logo')->value('value');
                        @endphp
                        @if($logo)
                            <img src="{{ asset($logo) }}" alt="Logo" class="h-full w-full object-contain p-1">
                        @else
                            <img src="{{ asset('assets/web/logo/cke-logomark.png') }}" alt="Cipta Kriya Engineering" class="h-full w-full object-contain p-1">
                        @endif
                    </div>
                    <div class="leading-tight">
                        <div class="font-extrabold text-[15px] tracking-tight" style="font-family:'Archivo',sans-serif;color:var(--cke-navy-700,#122b3f);">CIPTA KRIYA</div>
                        <div class="text-[11px] font-semibold tracking-[0.16em] uppercase" style="font-family:'Archivo',sans-serif;color:var(--cke-blue-600,#167ea7);">ENGINEERING</div>
                        <div class="text-[10px] font-medium uppercase tracking-[0.18em] mt-0.5 text-gray-400" style="font-family:'IBM Plex Mono',monospace;">CMS Admin</div>
                    </div>
                </a>
        ...
        <main class="flex-1 flex flex-col h-screen overflow-hidden bg-white">
            ...
            <div class="flex-1 overflow-y-auto p-8">
                ...
                <footer class="mt-12 text-center text-xs text-gray-400 pb-8">
                    &copy; {{ date('Y') }} PT. Cipta Kriya Engineering. CMS Admin Panel v1.0
                </footer>
            </div>
        </main>
    </div>
